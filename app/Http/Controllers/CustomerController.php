...
class CustomerController extends Controller
{
    ...
    public function getInvestCard()
    {
        return view('cus.investcard');
    }
    public function postInvestCard()
    {
        $this->validate(request(), [
            'amount' => 'required|integer|min:3600',
            'rate' => 'required',
            //'proof' => 'required|image|mimes:jpeg,jpg,png|max:1024'
        ]);

        return $this->start(request()->amount, request()->rate, Auth::user()->email);
    }
}